# Legal Disclaimer

The contents of this repository, including scripts, scenarios, and detection rules, are provided for educational, research purposes only. 

## 1. Lab Use Only & Written Permission Requirements

All tools and scripts are intended to be executed **in isolated lab environments**. Do not run any scripts against live systems, networks, or targets you do not own or have explicit permission to test.

### Written Permission Requirements

- **Explicit written permission** must be obtained from the system owner or authorized administrator before executing any code against non-lab environments
- Permission must be in writing (email or signed document)
- Permission must clearly state:
  - The specific systems/networks authorized for testing
  - The scope of testing activities
  - The duration of authorization
  - The responsible parties and their contact information
- The repository owner assumes **no liability** for actions taken without proper written authorization
- Verbal permission is insufficient; always obtain documentation

## 2. Limitation of Liability

The repository owner [Secwexen](https://github.com/secwexen) **assumes no responsibility** for any damage, misuse, or legal consequences resulting from the use of these materials:

- Damage caused by improper or illegal use   
- Actions performed by third parties using this code

Use them responsibly and ethically.

## 3. Compliance & Legal Responsibility

Users must comply with all local laws and organizational policies when using any materials from this repository. Unauthorized access to systems or networks is illegal and strictly prohibited.

## 4. Reference & Learning Purposes

Detection rules and attack scenarios are **for reference and learning purposes**. They may require adaptation before real-world application.

## 5. Disclaimer of Warranty

This repository is provided **“as is”** and **"as available"**, without warranty of any kind, express or implied.

## 6. Intellectual Property & Licensing

This repository and its contents are protected by copyright and applicable intellectual property laws.

### License Terms

- The specific license under which this repository is distributed is detailed in the [LICENSE](LICENSE) file
- Users must comply with all license terms and conditions
- Redistribution, modification, and commercial use are subject to license restrictions
- Attribution and license notices must be preserved in any derivative works
- If no explicit license is provided, all rights are reserved by the repository owner

### Third-Party Intellectual Property

- This repository may contain references to third-party tools and frameworks
- Users are responsible for reviewing and complying with the licenses of any third-party resources they use
- The repository owner is not liable for any intellectual property disputes arising from third-party usage

## 7. Acknowledgement & Acceptance

By accessing, cloning, or using this repository, you acknowledge that you have read, understood, and agreed to all terms listed in this disclaimer. If you do not agree with these terms, you must immediately cease using this repository and delete all copies from your systems.

**Last Updated:** September 1, 2026
